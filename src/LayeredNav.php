...
class LayeredNav extends WC_Widget_Layered_Nav
{
        ...
        if ( 0 < count( $terms ) ) {
            ...
            if ( 'dropdown' == $display_type ) {
                ...
                foreach ( $terms as $term ) {
                    ...
                    if (
                        'and' == $query_type
                        || !isset($_chosen_attributes[$term->taxonomy])
                        || 1 < count($_chosen_attributes)
                    ) {
                    /* END RIGHT FILTER */
...
//                        if ( 0 == $count && ! $option_is_set ) {
                    ...
                    if ( defined( 'SHOP_IS_ON_FRONT' ) ) {
                        ...
                        // cached term links
                        $hashTermLink = get_query_var('term') . '-' . get_query_var('taxonomy');
                        $link = $wp_object_cache->get($hashTermLink, self::CACHE_GROUP_TERM_LINK);
                        if(!$link) {
                            $link = get_term_link( get_query_var('term'), get_query_var('taxonomy') );
                            $wp_object_cache->set($hashTermLink, $link, self::CACHE_GROUP_TERM_LINK);
                        }
                    }
                    ...
                    if ( isset( $_chosen_attributes[ $taxonomy ] ) && is_array( $_chosen_attributes[ $taxonomy ]['terms'] ) && in_array( $term->term_id, $_chosen_attributes[ $taxonomy ]['terms'] ) ) {
                        ...
                        if ( sizeof( $current_filter ) > 1 ) {
                            ...
                            sort($current_filter_without_this);
                    ...
                    echo '<input type="checkbox" ' . ($isChecked ? 'checked' : '') . '/> ';
    ...
    public function widget_start( $args, $instance ) {
        ...
        ?>
            <div class="filter-title" data-target="#filter-<?php echo $instance['attribute']; ?>" data-toggle="collapse"
                 aria-expanded="true">
